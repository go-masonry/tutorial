...
func (s *workshopSuite) TestAcceptCar() {
	_, err := s.controller.AcceptCar(context.Background(), &api.Car{
		Number:    "1234",
		Owner:     "test owner",
		BodyStyle: api.Car_PHAETON,
		Color:     "cyan",
	})
	s.NoError(err)
...
func (s *workshopSuite) TestPaintCar() {
	...
	s.NoError(err)
	// test no such car
	_, err = s.controller.PaintCar(context.Background(), &api.PaintCarRequest{CarNumber: "fake car number"})
	s.Error(err)
	// now paint
	_, err = s.controller.PaintCar(context.Background(), &api.PaintCarRequest{
		CarNumber:    "1234567",
		DesiredColor: "orange",
	})
	s.NoError(err)
...
func (s *workshopSuite) TestRetrieveCar() {
	// No car
	_, err := s.controller.RetrieveCar(context.Background(), &api.RetrieveCarRequest{CarNumber: "not yet there"})
	...
	s.NoError(err)
	_, err = s.controller.RetrieveCar(context.Background(), &api.RetrieveCarRequest{CarNumber: "12345"})
	s.EqualError(err, "car 12345 is not painted")
	// Now paint the car and get it
	err = s.carDB.PaintCar(context.Background(), "12345", "black")
	s.NoError(err)
	carProto, err := s.controller.RetrieveCar(context.Background(), &api.RetrieveCarRequest{CarNumber: "12345"})
	s.NoError(err)
	s.Equal("black", carProto.GetColor())
	s.Equal("12345", carProto.GetNumber())
	s.Equal("test owner", carProto.GetOwner())
	s.Equal(api.Car_SEDAN, carProto.GetBodyStyle())
}
...
func (s *workshopSuite) TestCarPainted() {
	...
	s.NoError(err)
	_, err = s.controller.CarPainted(context.Background(), &api.PaintFinishedRequest{
		CarNumber:    "123456",
		DesiredColor: "indigo",
	})
	s.NoError(err)
...
func (s *workshopSuite) fxOptions() fx.Option {
	return fx.Options(
		...
		common.ViperFxOption(s.pwd+"/../../config/config.yml", s.pwd+"/../../config/config_test.yml"),
		common.LoggerFxOption(),
